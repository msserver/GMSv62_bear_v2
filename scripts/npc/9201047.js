...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendSimple("金錢倍數太高，人物只能放20億嗎?\r\n\用小熊獨特的屁屁壓縮技術可以幫你把20億變成1個幸運草!!\r\n\請選擇服務:\r\n\#L1##r錢換成獎盃#l\r\n\#L2##r獎盃換成錢#l#k \r\n\\r\n\獎盃的圖片:#i4000038#")						
		} else if (selection == 1) {
			if(cm.getMeso() >= 2000000000) {
				cm.gainMeso(-2000000000);
				cm.gainItem(4001126, 1);
				cm.sendOk("#e好了,看看物品欄內有沒有拿到#i4000038#吧!");
		} else {
				cm.sendOk("#e你錢不到20億我怎麼幫你壓縮呢?");
				cm.dispose();
		}
		} else if (selection == 2) {	
			if (cm.haveItem(4000038) >= 1 && cm.getMeso() < 347474329) {
				cm.gainItem(4000038, -1);
				cm.gainMeso(2000000000);
				cm.sendOk("#e好了,看看是不是有拿到20億!");
		} else {
				cm.sendOk("#e你根本沒有 #r 楓葉 #k #i4000038# 這個東西或者你的錢超過 #r 347474329元 #r 如果真的換成功的話其他的錢會被系統吃掉的");
		}
				cm.dispose();
		}
	}
