...
//                    }
    ...
    public void levelUp() {
        ...
        level += 1;
          if(getLevel() == 200) {
            MaplePacket packet = MaplePacketCreator.serverNotice(0, " 恭喜玩家 " + getName() + " 達到最高等級200級.讓我們一起熱烈的祝賀！");
            try
            {
        ...
        silentPartyUpdate();
        guildUpdate();
    if (this.getLevel()==70 && (job == MapleJob.SPEARMAN)) {
this.changeJob(job.DRAGONKNIGHT);
} else if (this.getLevel()==70 && (job == MapleJob.PAGE)) {
this.changeJob(job.WHITEKNIGHT);
} else if (this.getLevel()==70 && (job == MapleJob.FIGHTER)) {
this.changeJob(job.CRUSADER);
} else if (this.getLevel()==70 && (job == MapleJob.CLERIC)) {
this.changeJob(job.PRIEST);
} else if (this.getLevel()==70 && (job == MapleJob.FP_WIZARD)) {
this.changeJob(job.FP_MAGE);
} else if (this.getLevel()==70 && (job == MapleJob.IL_WIZARD)) {
this.changeJob(job.IL_MAGE);
} else if (this.getLevel()==70 && (job == MapleJob.CROSSBOWMAN)) {
this.changeJob(job.SNIPER);
} else if (this.getLevel()==70 && (job == MapleJob.HUNTER)) {
this.changeJob(job.RANGER);
} else if (this.getLevel()==70 && (job == MapleJob.ASSASSIN)) {
this.changeJob(job.HERMIT);
} else if (this.getLevel()==70 && (job == MapleJob.BANDIT)) {
this.changeJob(job.CHIEFBANDIT);
} else if (this.getLevel()==70 && (job == MapleJob.GUNSLINGER)) {
this.changeJob(job.OUTLAW);
} else if (this.getLevel()==70 && (job == MapleJob.BRAWLER)) {
this.changeJob(job.MARAUDER);
} else if (this.getLevel()==120 && (job == MapleJob.CRUSADER)) {
this.changeJob(job.HERO);
} else if (this.getLevel()==120 && (job == MapleJob.WHITEKNIGHT)) {
this.changeJob(job.PALADIN);
} else if (this.getLevel()==120 && (job == MapleJob.DRAGONKNIGHT)) {
this.changeJob(job.DARKKNIGHT);
} else if (this.getLevel()==120 && (job == MapleJob.PRIEST)) {
this.changeJob(job.BISHOP);
} else if (this.getLevel()==120 && (job == MapleJob.FP_MAGE)) {
this.changeJob(job.FP_ARCHMAGE);
} else if (this.getLevel()==120 && (job == MapleJob.IL_MAGE)) {
this.changeJob(job.IL_ARCHMAGE);
} else if (this.getLevel()==120 && (job == MapleJob.SNIPER)) {
this.changeJob(job.MARKSMAN);
} else if (this.getLevel()==120 && (job == MapleJob.RANGER)) {
this.changeJob(job.BOWMASTER);
} else if (this.getLevel()==120 && (job == MapleJob.HERMIT)) {
this.changeJob(job.NIGHTLORD);
} else if (this.getLevel()==120 && (job == MapleJob.CHIEFBANDIT)) {
this.changeJob(job.SHADOWER);
} else if (this.getLevel()==120 && (job == MapleJob.OUTLAW)) {
this.changeJob(job.CORSAIR);
} else if (this.getLevel()==120 && (job == MapleJob.MARAUDER)) {
this.changeJob(job.BUCCANEER);
}
