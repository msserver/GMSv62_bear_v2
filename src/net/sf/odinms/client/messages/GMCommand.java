            ...
            try {
                ...
                if (target != null) {
                    if (target.gmLevel() < 3 || player.gmLevel() > 4) {
                        ...
                        try {
                            ChannelServer.getInstance(c.getChannel()).getWorldInterface().broadcastMessage(player.getName(), MaplePacketCreator.serverNotice(0, readableTargetName + " 玩家使用非法外掛練功,已經被永久鎖IP,請玩家不要冒著風險使用非法程式。" + originalReason + ".").getBytes());
                        } catch (RemoteException e) {
                            cserv.reconnectWorld();
                        }
                    } else {
                        mc.dropMessage("你被GM鎖了~送你去地獄.");
                    }
                } else {
